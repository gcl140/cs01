...
def update_ball_position():
    global ball_x, ball_y, go_right, go_left, called_first, start_temp,start_temp_x, random_start

    if random_start:
        if called_first:
            start_temp = randint(0, 8)
            start_temp = randint(0, 8)
            # ball_y += start_temp
            called_first = False
        ball_y += start_temp
...
def collide_paddle():
    global paddle_1_y, ball_y, ball_x, go_right, go_left, go_up, go_down, start_game, random_start
    # if ball_x == 10:
    if ball_x != 200:
        start_game = False
    if ball_x >= 400:
    ...
    # if paddle_1_y <= ball_y and paddle_1_y + 100 >= ball_y and ball_x <= 10: 
        go_left, random_start = False, False
        go_right = True

    # elif paddle_2_y <= ball_y and paddle_2_y + 100 >= ball_y and ball_x == 390: 
    if paddle_2_y <= ball_y and paddle_2_y + 100 >= ball_y and ball_x == 390: 
        go_right, random_start = False, False
        go_left = True

    if ball_y >= 400:
        # go_right, go_left, go_up = False, False, False
        random_start = False
        go_up = True
